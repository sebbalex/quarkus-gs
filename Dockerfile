...
RUN \
  --mount=type=cache,target=/code/quarkus-gs/m2,sharing=locked \
  ["./mvnw", "verify", "clean", "-Dmaven.repo.local=/code/quarkus-gs/m2", "--fail-never"]

RUN \
  --mount=type=cache,target=/code/quarkus-gs/m2,sharing=locked \
  ./mvnw -f pom.xml -B package -Dmaven.repo.local=/code/quarkus-gs/m2 -Dmaven.test.skip=true
